...
export function DCLayout({ children }: { children: ReactNode }) {
  const { user, role, signOut, displayName, isAdmin } = useAuth();

  const nav = [
    { to: "/dc", label: "DC Entry", icon: FileText, show: true },
  ].filter((n) => n.show);
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      <header className="border-b border-border/60 backdrop-blur bg-background/80 sticky top-0 z-40">
        <div className="container flex h-16 max-w-full items-center gap-3 px-3 sm:px-6 lg:px-8">
          <Link to="/" className="flex min-w-0 items-center gap-2 font-bold group">
            ...
            <span className="truncate text-base tracking-tight sm:text-lg">Apex<span className="text-primary"> DC Entry</span></span>
      ...
      <main className="container max-w-full flex-1 px-3 py-4 sm:px-6 md:py-6 lg:px-8">{children}</main>
    </div>
  );
}
